...
<div class="awards">
  ...
  <div id="podium">
    ...
    <img
      ...
      on:click={() => gotoManager(champion.rosterID)}
      alt="champion"
    />
    ...
    <span
      ...
      on:click={() => gotoManager(champion.rosterID)}
      >{@html getNames(champion.name, champion.rosterID)}</span
    >

    <img
      src={second.avatar}
      class="second champ clickable"
      on:click={() => gotoManager(second.rosterID)}
      alt="2nd"
    />
    <span
      class="label secondLabel clickable"
      on:click={() => gotoManager(second.rosterID)}
      >{@html getNames(second.name, second.rosterID)}</span
    >

    <img
      src={third.avatar}
      class="third champ clickable"
      on:click={() => gotoManager(third.rosterID)}
      alt="3rd"
    />
    <span
      class="label thirdLabel clickable"
      on:click={() => gotoManager(third.rosterID)}
      >{@html getNames(third.name, third.rosterID)}</span
    >
  </div>
  <div class="divisions">
      ...
      {#if division.manager}
        <div class="division">
          ...
          <div class="leaderBlock">
            <img
              src={division.manager.avatar}
              class="divisionLeader clickable"
              on:click={() => gotoManager(division.manager.rosterID)}
    ...
    <div class="toiletParent">
      ...
      <div class="toiletBowl">
        <img
          ...
          on:click={() => gotoManager(toilet.rosterID)}
      ...
      <span
        class="genLabel clickable"
        on:click={() => gotoManager(toilet.rosterID)}
        >{@html getNames(toilet.name, toilet.rosterID)}</span
      >
    </div>
  {/if}
</div>
