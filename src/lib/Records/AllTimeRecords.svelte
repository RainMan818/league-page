<script>
  ...
  const setRankingsData = (lRR) => {
    ...
    for (const key in lRR) {
      ...
      const denominator =
        leagueRosterRecord.wins +
          leagueRosterRecord.ties +
          leagueRosterRecord.losses >
        0
          ? leagueRosterRecord.wins +
            leagueRosterRecord.ties +
            leagueRosterRecord.losses
      ...
      winPercentages.push({
        ...
        percentage: round(
          ((leagueRosterRecord.wins + leagueRosterRecord.ties / 2) /
            denominator) *
            100
        ),
      ...
      if (leagueRosterRecord.potentialPoints) {
        lineupIQ.iq = round(
          (leagueRosterRecord.fptsFor / leagueRosterRecord.potentialPoints) *
            100
        );
