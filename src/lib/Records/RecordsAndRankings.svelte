      ...
      <Head>
        <Row>
          <Cell class="header" colspan="4"
            >{prefix} Single Week Scoring Records</Cell
          >
        </Row>
        <Row>
      ...
      <Body>
        {#each weekRecords as leagueWeekRecord, ix}
          <Row>
            ...
            <Cell
              class="cellName"
              on:click={() => gotoManager(leagueWeekRecord.rosterID)}
            >
              ...
              {#if !allTime && cleanName(leagueWeekRecord.manager.name) != cleanName(currentManagers[leagueWeekRecord.rosterID].name)}
                <div class="curRecordManager">
                  ({currentManagers[leagueWeekRecord.rosterID].name})
                </div>
              {/if}
            </Cell>
            <Cell
  ...
  {#if weekLows && weekLows.length}
    <DataTable class="recordTable">
      <Head>
        <Row>
          <Cell class="header" colspan="4"
            >{prefix} Single Week Scoring Lows</Cell
          >
        </Row>
        <Row>
          <Cell class="header rank" />
      ...
      <Body>
        {#each weekLows as leagueWeekLow, ix}
          <Row>
            ...
            <Cell
              class="cellName"
              on:click={() => gotoManager(leagueWeekLow.rosterID)}
            >
              ...
              {#if !allTime && cleanName(leagueWeekLow.manager.name) != cleanName(currentManagers[leagueWeekLow.rosterID].name)}
                <div class="curRecordManager">
                  ({currentManagers[leagueWeekLow.rosterID].name})
                </div>
              {/if}
            </Cell>
            <Cell
      ...
      <Body>
        {#each seasonLongRecords as mostSeasonLongPoint, ix}
          <Row>
            ...
            <Cell
              class="cellName"
              on:click={() => gotoManager(mostSeasonLongPoint.rosterID)}
            >
      ...
      <Body>
        {#each seasonLongLows as leastSeasonLongPoint, ix}
          <Row>
            ...
            <Cell
              class="cellName"
              on:click={() => gotoManager(leastSeasonLongPoint.rosterID)}
            >
        ...
        {#each blowouts as blowout, ix}
          <Row>
            ...
            <Cell class="cellName center differentialName">
              <div
                class="center"
                on:click={() => gotoManager(blowout.home.rosterID)}
              >
                ...
                {#if !allTime && cleanName(blowout.home.manager.name) != cleanName(currentManagers[blowout.home.rosterID].name)}
                  <div class="curRecordManager">
                    ({currentManagers[blowout.home.rosterID].name})
                  </div>
                {/if}
              </div>
              vs
              <div
                class="center"
                on:click={() => gotoManager(blowout.away.rosterID)}
              >
                ...
                {#if !allTime && cleanName(blowout.away.manager.name) != cleanName(currentManagers[blowout.away.rosterID].name)}
                  <div class="curRecordManager">
                    ({currentManagers[blowout.away.rosterID].name})
                  </div>
                {/if}
              </div>
            </Cell>
        ...
        {#each closestMatchups as closestMatchup, ix}
          <Row>
            ...
            <Cell class="cellName center differentialName">
              <div
                class="center"
                on:click={() => gotoManager(closestMatchup.home.rosterID)}
              >
                {closestMatchup.home.manager.name} ({round(
                  closestMatchup.home.fpts
                )})
                {#if !allTime && cleanName(closestMatchup.home.manager.name) != cleanName(currentManagers[closestMatchup.home.rosterID].name)}
                  <div class="curRecordManager">
                    ({currentManagers[closestMatchup.home.rosterID].name})
                  </div>
                {/if}
              </div>
              vs
              <div
                class="center"
                on:click={() => gotoManager(closestMatchup.away.rosterID)}
              >
                {closestMatchup.away.manager.name} ({round(
                  closestMatchup.away.fpts
                )})
                {#if !allTime && cleanName(closestMatchup.away.manager.name) != cleanName(currentManagers[closestMatchup.away.rosterID].name)}
                  <div class="curRecordManager">
                    ({currentManagers[closestMatchup.away.rosterID].name})
                  </div>
                {/if}
              </div>
            </Cell>
...
{#if graphs.length}
  <BarChart maxWidth={innerWidth} {graphs} bind:curGraph />
{/if}
        ...
        <Head>
          <Row>
            <Cell class="header" colspan="6"
              >{prefix} Win Percentages Rankings</Cell
            >
          </Row>
          <Row>
            <Cell class="header" />
        ...
        <Body>
          {#each winPercentages as winPercentage, ix}
            <Row>
              ...
              <Cell
                class="cellName"
                on:click={() => gotoManager(winPercentage.rosterID)}
              >
                ...
                {#if !allTime && cleanName(winPercentage.manager.name) != cleanName(currentManagers[winPercentage.rosterID].name)}
                  <div class="curRecordManager">
                    ({currentManagers[winPercentage.rosterID].name})
                  </div>
                {/if}
              </Cell>
        ...
        <Body>
          {#each fptsHistories as fptsHistory, ix}
            <Row>
              ...
              <Cell
                class="cellName"
                on:click={() => gotoManager(fptsHistory.rosterID)}
              >
                ...
                {#if !allTime && cleanName(fptsHistory.manager.name) != cleanName(currentManagers[fptsHistory.rosterID].name)}
                  <div class="curRecordManager">
                    ({currentManagers[fptsHistory.rosterID].name})
                  </div>
                {/if}
              </Cell>
        ...
        <Body>
          {#each transactions as transaction, ix}
            <Row>
              ...
              <Cell
                class="cellName"
                on:click={() => gotoManager(transaction.rosterID)}
              >
                ...
                {#if !allTime && cleanName(transaction.manager?.name) != cleanName(currentManagers[transaction.rosterID].name)}
                  <div class="curRecordManager">
                    ({currentManagers[transaction.rosterID].name})
                  </div>
                {/if}
              </Cell>
