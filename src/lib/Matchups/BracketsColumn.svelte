  ...
  {#each matchCol as matchups, inx}
    <div
      class="match{matchups[0].m == selected ? ' selected' : ''}{matchups[0]
        .m && matchups[1].manager
        ? ' clickable'
        : ''}"
      bind:this={anchors[Math.floor(inx / 2)][inx % 2 == 0 ? "t" : "b"]}
      on:click={() => {
        changeSelection(matchups[0].m, matchups[1].manager);
      }}
    >
        ...
        <div class="manager">
          <div class="avatarPointsBlock">
            ...
            {#if matchup.manager || (!matchups.bye && !matchup.manager)}
              <img
                class="avatar{!matchups.bye && !matchup.manager
                  ? ' avatarBye'
                  : ''}"
                src={matchup.manager?.avatar || "/managers/question.jpg"}
                alt="team avatar"
              />
            {/if}
            {#if matchup.manager}
          ...
          <div class="name{matchups.bye && !matchup.manager ? ' bye' : ''}">
            {getPlayoffName(matchup.manager, matchups.bye)}
          </div>
        </div>
      {/each}
    </div>
