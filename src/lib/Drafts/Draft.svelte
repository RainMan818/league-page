        ...
        {#if draftPosition}
          <Cell class="draftTeam">
            <img
              ...
              on:click={() => gotoManager(draftPosition)}
              src={originalManagers[draftPosition].avatar}
              alt="{originalManagers[draftPosition].name} avatar"
            />
            <br />
            <span class="clickable" on:click={() => gotoManager(draftPosition)}
              >{originalManagers[draftPosition].name}{@html currentManagers &&
              cleanName(currentManagers[draftPosition].name) !=
                cleanName(originalManagers[draftPosition].name)
                ? `<br /><span class="curDraftName">(${currentManagers[draftPosition].name})</span>`
                : ""}</span
            >
          </Cell>
        {/if}
