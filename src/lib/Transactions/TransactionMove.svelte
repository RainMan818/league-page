<script>
  ...
  const resize = (width) => {
    x1 =
      origin?.getBoundingClientRect() && destination?.getBoundingClientRect()
        ? destination?.getBoundingClientRect().left >
          origin?.getBoundingClientRect().left
          ? origin?.getBoundingClientRect().right + originOffset - masterOffset
          : origin?.getBoundingClientRect().left - originOffset - masterOffset
        : 0;
    x2 =
      origin?.getBoundingClientRect() && destination?.getBoundingClientRect()
        ? destination?.getBoundingClientRect().left >
          origin?.getBoundingClientRect().left
          ? destination?.getBoundingClientRect().left - offset - masterOffset
          : destination?.getBoundingClientRect().right + offset - masterOffset
        : 0;
  };
...
<Row>
  {#each move as cell}
    {#if cell && cell.player}
      <Cell class="move {cell.type}">
        <span class="nameHolder">
          ...
          {/if}
          {cell.type == "Added" || cell.type == "Dropped"
            ? cell.type + `${cell.bid ? " " + cell.bid + "$" : ""}`
            : ""}
          {#if cell.type == "Added"}
          ...
          {/if}
          <span class="pos {players[cell.player].pos}"
            >{players[cell.player].pos}</span
          >
          <div
            class="playerAvatar"
            style={getAvatar(players[cell.player].pos, cell.player)}
          />
          <br />
          <span class="name" bind:this={origin}
    ...
    {:else if cell && cell.pick}
      <Cell class="move {cell.type}"
        ><span bind:this={origin} class="name"
          >{cell.pick.season} Round {cell.pick.round}{@html cell.pick
            .original_owner
            ? `<br /><span class="originalOwner">(${
                cell.pick.original_owner.original &&
                cleanName(cell.pick.original_owner.original) !=
                  cleanName(
                    currentManagers[cell.pick.original_owner.current].name
                  )
                  ? `${cell.pick.original_owner.original} [`
                  : ""
              }${currentManagers[cell.pick.original_owner.current].name}${
    ...
    {:else if cell && cell.budget}
      <Cell class="move {cell.type}"
        ><span bind:this={origin} class="name">{cell.budget.amount}</span></Cell
      >
    {:else if cell && cell == "destination"}
      <Cell class="move trade"
        ><span class="destination" bind:this={destination} /></Cell
      >
    {:else}
      <Cell class="trade" />
    {/if}
