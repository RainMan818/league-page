...
<DataTable class="transaction">
        ...
        <Cell class="transactTeam">
          {#if transaction.previousOwners && cleanName(transaction.previousOwners[ix].name) != cleanName(currentManagers[owner].name)}
            <img
              ...
              on:click={() => gotoManager(owner)}
              src={transaction.previousOwners[ix].avatar}
              alt="{transaction.previousOwners[ix].name} avatar"
            />
            <br /><span class="clickable" on:click={() => gotoManager(owner)}
              >{transaction.previousOwners[ix].name}</span
            >
          ...
          {:else}
            <img
              class="avatar clickable"
              on:click={() => gotoManager(owner)}
              src={currentManagers[owner].avatar}
              alt="{currentManagers[owner].name} avatar"
            />
            <br /><span class="clickable" on:click={() => gotoManager(owner)}
              >{currentManagers[owner].name}</span
            >
          {/if}
        </Cell>
      {/each}
  ...
  <Body class="moves">
    ...
    {/each}
    <Row>
      <Cell class="transact-date" colspan={transaction.rosters.length}
