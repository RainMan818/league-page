...
export const getLeagueRecords = async (refresh = false) => {
  ...
  while (curSeason && curSeason != 0) {
    ...
    // regular season data
    const { originalManagers, season, year, rS } = await processRegularSeason({
      leagueData,
      users,
      rosters,
      curSeason,
...
const processRegularSeason = async ({
  ...
  for (const matchupWeek of matchupsData) {
    const { sPR, r, mD, sW } = processMatchups({
      matchupWeek,
      originalManagers,
      seasonPointsRecord,
...
const analyzeRosters = ({
  ...
  if (
    ...
    return { rS: regularSeason, oM: originalManagers };
...
const digestBracket = ({
  ...
  for (let i = 0; i < bracket.length; i++) {
    ...
    const { sPR, r, mD, pSD } = processMatchups({
      matchupWeek,
      originalManagers,
      seasonPointsRecord,
