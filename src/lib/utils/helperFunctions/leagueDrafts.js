...
const buildFromScratch = (
  ...
  for (const pick of picks) {
    ...
    draft[pick.round - 1][draftOrder.indexOf(pick.roster_id)] =
      originalManagers[pick.owner_id].name;
  }
...
const completedNonAuction = ({
  ...
  for (const playerData of players) {
    const player = {
      ...
      avatar:
        playerData.metadata.position == "DEF"
          ? `background-image: url(https://sleepercdn.com/images/team_logos/nfl/${playerData.player_id.toLowerCase()}.png)`
          : `background-image: url(https://sleepercdn.com/content/nfl/players/thumb/${playerData.player_id}.jpg), url(https://sleepercdn.com/images/v2/icons/player_default.webp)`,
    };
